                ...
                <>
                    ...
                    >
                        <motion.div
                            ...
                            className='border-4 border-white gradient-full text-white p-6 rounded-xl w-full max-w-[38rem] width-laptop:max-w-3xl shadow-xl cursor-default relative overflow-hidden'
                        >
                            <div className='relative z-10'>
                                <X className='absolute right-0 top-0 cursor-pointer' onClick={() => setIsOpen(false)} />
                                <h3 className={`${audiowide.className} text-4xl width-laptop:text-5xl font-bold text-center`}>STATS & SHOP</h3>
                                <div className={`${audiowide.className} flex justify-between py-4 width-laptop:py-6`}>
                                    <p className='p-2 border-2 border-white rounded-md shadow-md text-[13px] width-laptop:text-base'>MONEY: ${profile?.money}</p>
                                    <p className='p-2 border-2 border-white rounded-md shadow-md text-[13px] width-laptop:text-base'>GAMES PLAYED: {profile?.games?.length}</p>
                                    <p className='p-2 border-2 border-white rounded-md shadow-md text-[13px] width-laptop:text-base'>
                                        GAMES WON: {profile?.games?.filter((item) => item.won).length}
                                    </p>
                                    <p className='p-2 border-2 border-white rounded-md shadow-md text-[13px] width-laptop:text-base'>
                                        GAMES LOST: {profile?.games?.filter((item) => !item.won).length}
                                    </p>
                                </div>
                                <h4 className={`${audiowide.className} text-xl width-laptop:text-2xl mb-2`}>ALLELES</h4>
                                <div className='flex flex-col gap-3 width-laptop:gap-5 height-tablet:px-10 width-laptop:height-tablet:px-10'>
                                    {alleleArray.map(({ type, unit, alleles, stats, color, letter }) => (
                                        <div className='grid grid-cols-[60px_1fr] gap-2 height-tablet:block' key={type}>
                                            <span className={`${orbitronBold.className} grid height-tablet:hidden place-items-center animated-text-gradient-${color}-full text-4xl uppercase`}>{type.substring(0, 1)}</span>
                                            <div key={type + color}>
                                                <h5 className={`${audiowide.className} hidden height-tablet:block text-lg animated-text-gradient-${color}-full mb-2 uppercase`}>
                                                    {type} ALLELES
                                                </h5>
                                                        ...
                                                        {Array.from({ length: 6 }).map((_, index) => {
                                                            ...
                                                            return (
                                                                <div
                                                                    ...
                                                                    } flex flex-col gap-0.5 justify-center items-center relative aspect-square text-sm width-laptop:text-base`}
                                                                >
                                                        ...
                                                        {Array.from({ length: 6 }).map((_, index) => {
                                                            ...
                                                            return (
                                                                ...
                                                                >
                                                                    ...
                                                                    <span className='text-xs width-laptop:text-base'>
                                                                        {unlocked ? `${stats[index].cost === 0 ? 'FREE' : 'PAID'}` : 'BUY'}
                                                                    </span>
                                                                </div>
                                                            );
                                                        })}
