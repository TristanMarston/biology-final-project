                ...
                <AnimatePresence>
                    {triviaGame.stage === 'instructions' && (
                        <div className='w-full h-full flex flex-col gap-6 items-center justify-center'>
                            <span className='flex'>
                                <CircleHelp className='mt-4 mr-8 text-white w-24 h-24 bg-[rgba(255,255,255,.4)] rounded-full' strokeWidth={2} />
                                <CircleHelp className='mt-7 mr-2 text-white w-24 h-24 bg-[rgba(255,255,255,.4)] rounded-full' strokeWidth={2} />
                                <CircleHelp className='-mt-3 mr-6 text-white w-24 h-24 bg-[rgba(255,255,255,.4)] rounded-full' strokeWidth={2} />
                                <CircleHelp className='mt-7 mr-4 text-white w-24 h-24 bg-[rgba(255,255,255,.4)] rounded-full' strokeWidth={2} />
                                <CircleHelp className='mt-3 mr-4 text-white w-24 h-24 bg-[rgba(255,255,255,.4)] rounded-full' strokeWidth={2} />
                                <CircleHelp className='-mt-2 text-white w-24 h-24 bg-[rgba(255,255,255,.4)] rounded-full' strokeWidth={2} />
                            </span>
                    ...
                    {triviaGame.stage === 'game-active' && triviaGame.currentQuestion !== null && (
                        <div className='w-full h-full flex items-center justify-center'>
                            ...
                            >
                                ...
                                <h4 className={`${orbitronBold.className} text-3xl text-center`}>{triviaGame.currentQuestion.question}</h4>
                                <div className='grid grid-cols-2 grid-rows-2 w-full gap-3'>
                                    {shuffledArray.map((which, index) => (
                                        <div
                                            ...
                                            className='rounded-2xl border-2 border-white shadow-lg flex items-center gap-2 bg-[#6fa9ff] hover:brightness-[1.1] transition-all p-4 cursor-pointer'
                                        >
                                            <div
                                                className={`${orbitronSemibold.className} ${
                                                    index === 0 || index === 2 ? 'brightness-[1.55]' : 'brightness-150'
                                                } p-4 rounded-full bg-[#4f78b6] shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] w-16 h-16 grid place-items-center text-xl uppercase`}
                                            >
                                                {index === 0 ? 'A' : index === 1 ? 'B' : index === 2 ? 'C' : index === 3 && 'D'}
                                            </div>
                                            <span className={`${orbitronSemibold.className} text-lg`}>{triviaGame.currentQuestion?.answers[which as 'a' | 'b' | 'c' | 'd'].answer}</span>
                                        </div>
                                    ))}
                                </div>
                            </motion.div>
                    ...
                    {triviaGame.stage === 'game-ended' && (
                        ...
                        >
                            {triviaGame.gameWon ? <Crown className='w-72 h-72 text-white' strokeWidth={3} /> : <Frown className='w-72 h-72 text-white' strokeWidth={3} />}
                            <span className={`${orbitronSemibold.className} w-[60%] text-center text-lg flex flex-col gap-1.5`}>
                                <span className='text-xl'>You {triviaGame.gameWon ? 'won' : 'lost'}!</span>
